...
@click.option('--n-clusters', '-n', type=int, help='Number of clusters (required for kmeans/spectral)')
@click.option('--method', '-m', type=click.Choice(['kmeans', 'hdbscan', 'spectral', 'dbscan']), 
              default='kmeans', help='Clustering method')
@click.option('--model', type=click.Choice(['clip', 'dinov2', 'dinov2-large']), 
              default='dinov2', help='Embedding model')
@click.option('--batch-size', '-b', type=int, default=32, help='Batch size for embedding extraction')
...
@click.option('--n-samples', type=int, default=5, help='Number of representative samples per cluster')
...
@click.option('--export-format', type=click.Choice(['csv', 'json']), default='csv', 
...
def annotate(
    ...
    try:
        ...
        console.print(f"[green]✓[/green] Found representatives for {len(representatives)} clusters\n")
        
        class_names = session.label_all_clusters(
            image_paths, labels, representatives, stats
        )

...
class InteractiveLabelingSession:
    ...
    def get_class_name_for_cluster(
        ...
        self.console.print("[bold cyan]Review the images in your browser, then return here to label.[/bold cyan]\n")
        ...
        while True:
            class_name = Prompt.ask(
                f"[bold green]Class name for Cluster {cluster_id}[/bold green]",
                default=""
            ).strip()
    ...
    def label_all_clusters(
        ...
        sorted_clusters = sorted(
            cluster_stats['cluster_sizes'].items(),
            key=lambda x: x[1],
            reverse=True
        )
        ...
        for cluster_id, cluster_size in sorted_clusters:
            ...
            if class_name:
                ...
                self.console.print(f"[green]✓ Cluster {cluster_id} labeled as '{class_name}'[/green]\n")
            else:

...
def validate_directory(
    path: Union[str, Path], must_exist: bool = True, must_be_empty: bool = False
) -> Path:
    path = Path(path)
...
def validate_model_name(model_name: str) -> str:
    if model_name not in MODEL_CONFIGS:
        raise ValidationError(
            f"Invalid model name: {model_name}. " f"Available models: {list(MODEL_CONFIGS.keys())}"
        )
    return model_name
...
def validate_n_clusters(n_clusters: Optional[int], method: str, n_samples: int) -> Optional[int]:
    config = CLUSTERING_CONFIGS[method]

    if config["requires_n_clusters"] and n_clusters is None:
        raise ValidationError(f"Clustering method '{method}' requires n_clusters to be specified")

    if n_clusters is not None:
        if n_clusters < 2:
            raise ValidationError(f"n_clusters must be at least 2, got {n_clusters}")
...
def validate_embeddings(embeddings: np.ndarray) -> np.ndarray:
    ...
    if embeddings.shape[0] < 2:
        raise ValidationError(f"Need at least 2 samples for clustering, got {embeddings.shape[0]}")
...
def validate_labels(labels: np.ndarray, n_samples: int) -> np.ndarray:
    if not isinstance(labels, np.ndarray):
        raise ValidationError(f"Labels must be numpy array, got {type(labels)}")

    if labels.ndim != 1:
        raise ValidationError(f"Labels must be 1D array, got shape {labels.shape}")
...
def validate_class_names(class_names: dict, labels: np.ndarray) -> dict:
    if not isinstance(class_names, dict):
        raise ValidationError(f"class_names must be a dictionary, got {type(class_names)}")
    ...
    for label, name in class_names.items():
        ...
        if not isinstance(name, str):
            raise ValidationError(f"Class name must be string, got {type(name)} for class {label}")

        if not name.strip():
            raise ValidationError(f"Class name cannot be empty for cluster {label}")

        if label not in labels and label != -1:
            import warnings

            warnings.warn(f"Class label {label} ('{name}') not found in clustering results")

    invalid_chars = ["/", "\\", ":", "*", "?", '"', "<", ">", "|"]
    for label, name in class_names.items():
        if any(char in name for char in invalid_chars):
            raise ValidationError(
                f"Class name '{name}' contains invalid characters. " f"Avoid: {invalid_chars}"
            )

    return class_names


def validate_split_ratios(train_ratio: float, val_ratio: float, test_ratio: float) -> tuple:
...
def validate_representative_samples(n_samples: int, cluster_size: int) -> int:
    if n_samples < 1:
...
def validate_export_format(format: str) -> str:
    ...
    if format.lower() not in valid_formats:
        raise ValidationError(
            f"Invalid export format: {format}. " f"Valid formats: {valid_formats}"
        )

    return format.lower()


def validate_images_list(images: List, min_images: int = 2) -> List:
    if not images:
        raise ValidationError("Images list is empty")

    if len(images) < min_images:
        raise ValidationError(f"Need at least {min_images} images, got {len(images)}")

    for i, img in enumerate(images):
        if not isinstance(img, Image.Image):
            raise ValidationError(f"Image at index {i} is not a PIL Image object, got {type(img)}")
...
def validate_device(device: str) -> str:
    ...
    if device not in valid_devices:
        raise ValidationError(f"Invalid device: {device}. Valid devices: {valid_devices}")

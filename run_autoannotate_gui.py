...
class AutoAnnotateGUI:
    ...
    def create_widgets(self):
        ...
        tk.Button(
            frame, text="Browse...", command=self.browse_input, bg="#4CAF50", fg="white", font=("Arial", 9)
        ).grid(row=0, column=2)
        ...
        tk.Button(
            frame, text="Browse...", command=self.browse_output, bg="#4CAF50", fg="white", font=("Arial", 9)
        ).grid(row=1, column=2)
        ...
        self.progress = ttk.Progressbar(
            self.root, length=500, mode='indeterminate'
        )
        ...
        try:
            ...
            if len(images) < self.n_clusters.get() * 3:
                response = messagebox.askyesno(
                    ...
                    f"You have {len(images)} images but requested {self.n_clusters.get()} clusters.\n\n"
                    f"Recommended: At least {self.n_clusters.get() * 3} images for good clustering.\n\n"
                    f"Continue anyway?"
                )
                if not response:
